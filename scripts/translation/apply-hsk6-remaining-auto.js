...
const pending = english.filter((item) => item.level === 6 && !hasSpanishEntry(item));
...
(async () => {
  ...
  for (let i = 0; i < pending.length; i += batchSize) {
    ...
    console.log(
      `HSK6 batch ${String(batchNumber).padStart(2, '0')} auto-applied. Inserted=${chunkInserted}, FallbackToEnglish=${chunkFallback}`
    );
  }
  ...
  console.log(`HSK6 remaining auto-complete done. Inserted=${inserted}, FallbackToEnglish=${fallbackToEnglish}, Total=${spanish.length}`);
})();
