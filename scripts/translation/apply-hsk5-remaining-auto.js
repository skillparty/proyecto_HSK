...
const pending = english.filter((item) => item.level === 5 && !hasSpanishEntry(item));
...
(async () => {
  ...
  for (let i = 0; i < pending.length; i += batchSize) {
    ...
    const batchNumber = startBatch + Math.floor(i / batchSize);
    ...
    console.log(`HSK5 batch ${String(batchNumber).padStart(2, '0')} auto-applied. Inserted=${chunkInserted}, FallbackToEnglish=${chunkFallback}`);
  }
  ...
  console.log(`HSK5 remaining auto-complete done. Inserted=${inserted}, FallbackToEnglish=${fallbackToEnglish}, Total=${spanish.length}`);
})();
