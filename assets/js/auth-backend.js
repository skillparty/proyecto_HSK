...
class BackendAuth {
  ...
  isStaticHosting() {
    ...
    return (
      hostname.includes("github.io") || window.location.protocol === "file:"
    );
  }
  ...
  async logout() {
    ...
    const logoutText =
      ...
      typeof window.languageManager.getText === "function"
        ? window.languageManager.getText("loggedOut")
  ...
  async apiCall(endpoint, options = {}) {
    if (endpoint.startsWith("/api/") && !this.legacyBackendApiEnabled) {
      ...
      return {
        ...
        json: async () => ({
          skipped: true,
          reason: "static-hosting-no-backend",
        }),
      };
    }
  ...
  showUserProfile() {
    ...
    authContainer.innerHTML = `
            <div class="user-profile">
                ...
                <div class="user-info">
                    <div class="user-name">${this.escapeHtml(this.currentUser.name || this.currentUser.username)}</div>
                    <div class="user-login">@${this.escapeHtml(this.currentUser.username)}</div>
  ...
  showWelcomeMessage() {
    if (this.currentUser) {
      ...
      this.showMessage(
        `${welcomeText}, ${this.currentUser.name || this.currentUser.username}!`,
      );
    }
  }
  ...
  debugAuthState() {
    console.log("🔍 Auth Debug State:", {
      ...
      authContainerHTML:
        document.getElementById("auth-container")?.innerHTML.substring(0, 100) +
        "...",
    });
  }
