...
class FirebaseClient {
  ...
  async initialize() {
    try {
      ...
      if (!window.firebaseAuth || !window.firebaseDb) {
        ...
        throw new Error(
          "Firebase configuration not initialized after 10 retries",
        );
      }
  ...
  async getLeaderboardStats() {
    try {
      ...
      const stats = {
        ...
        total_words_studied:
          data.reduce(
            (sum, user) => sum + (user.total_words_studied || 0),
            0,
          ) || 0,
        avg_words_per_user:
          data.length > 0
        ...
        weekly_active_users:
          data.filter((user) => {
            ...
            const lastActivity = user.last_studied.toDate
              ? user.last_studied.toDate()
              : new Date(user.last_studied);
    ...
    try {
      ...
      return {
        ...
        accuracy_rate: Math.round(
          ((data[userIndex].correct_answers || 0) /
            (data[userIndex].total_words_studied || 1)) *
            100,
        ),
      };
    } catch (error) {
  ...
  async getUserStatistics() {
    ...
    try {
      ...
      const stats = {
        totalStudied:
          data?.reduce(
            (sum, level) => sum + (level.total_words_studied || 0),
            0,
          ) || 0,
        correctAnswers:
          data?.reduce((sum, level) => sum + (level.correct_answers || 0), 0) ||
          0,
        incorrectAnswers:
          data?.reduce(
  ...
  async updateProgress(hskLevel, isCorrect, timeSpent = 0) {
    ...
    // Verify increment is available (requires Firebase Firestore v9+)
    if (!window.FirebaseSDK?.increment) {
      console.warn(
        "⚠️ FieldValue.increment not available, falling back to read-modify-write",
      );
      return this._updateProgressLegacy(hskLevel, isCorrect, timeSpent);
    }
    ...
    try {
      ...
      // Use atomic increment for counters — avoids race conditions on multi-tab/multi-write
      const baseUpdate = {
        user_id: this.user.uid,
        hsk_level: parseInt(hskLevel),
        total_words_studied: sdk.increment(1),
        total_time_spent: sdk.increment(timeSpent || 0),
        last_studied: sdk.serverTimestamp(),
      };

      if (isCorrect) {
        baseUpdate.correct_answers = sdk.increment(1);
        baseUpdate.current_streak = sdk.increment(1);
      } else {
        baseUpdate.incorrect_answers = sdk.increment(1);
        baseUpdate.current_streak = 0; // reset on incorrect answer
      }

      await sdk.setDoc(docRef, baseUpdate, { merge: true });

      // best_streak requires a read-compare-write — use a transaction for atomicity
      if (isCorrect) {
        await sdk.runTransaction(this.db, async (transaction) => {
  ...
  /** @private Fallback for environments where increment() is unavailable */
  async _updateProgressLegacy(hskLevel, isCorrect, timeSpent = 0) {
    const sdk = window.FirebaseSDK;
    const docId = `${this.user.uid}_hsk${hskLevel}`;
    const docRef = sdk.doc(this.db, "user_progress", docId);
    const docSnap = await sdk.getDoc(docRef);
    ...
    data.total_time_spent += timeSpent || 0;
    data.last_studied = sdk.serverTimestamp();
  ...
  async saveWordProgress(wordData) {
    ...
    try {
      ...
      const wordIdentifier =
        wordData.word_id || wordData.id || wordData.character;
      if (!wordIdentifier) {
        console.warn(
          "⚠️ saveWordProgress: wordData has no identifier (word_id, id, or character). Skipping.",
        );
        return;
      }
      const docId = `${this.user.uid}_${wordIdentifier}`;
  ...
  async getLeaderboard(type = "total_words", limitNum = 50) {
    try {
      ...
      const formattedData = data.map((entry, index) => ({
        ...
        avatar_url:
          entry.avatar_url ||
          `https://ui-avatars.com/api/?name=${entry.username || "A"}&background=random`,
        total_studied: entry.total_words_studied || 0,
        accuracy_rate: Math.round(
          ((entry.correct_answers || 0) / (entry.total_words_studied || 1)) *
            100,
        ),
  ...
  showUserProfile(container) {
    ...
    container.innerHTML = `
            <div class="user-profile">
                <div class="user-avatar-container">
                    <img src="${window.hskSafeHttpsUrl(photoURL, "https://github.com/github.png")}" alt="Avatar" class="user-avatar" data-fallback-src="https://github.com/github.png">
                </div>
                <div class="user-info">
                ...
                <button class="logout-btn" data-auth-action="logout" title="${this.t("logout")}">
    ...
    this.bindAuthActions(container);
  }

  // Reemplaza los onclick/onerror inline, que impiden aplicar una CSP sin
  // 'unsafe-inline'. Se llama tras cada innerHTML: los elementos son nuevos en
  // cada render, así que no se acumulan listeners.
  bindAuthActions(container) {
    const logoutBtn = container.querySelector('[data-auth-action="logout"]');
    if (logoutBtn) {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", () => this.signInWithGitHub());
    }
  ...
  showLoginButton(container) {
    container.innerHTML = `
            <div class="auth-guest">
                <button class="github-login-btn" data-auth-action="login" title="Iniciar sesión con GitHub">
